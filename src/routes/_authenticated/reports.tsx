...
function ReportsPage() {
    ...
    queryFn: async () => {
      const [txs, cats, fees, payments] = await Promise.all([
        supabase
          ...
          .select("type, amount, transaction_date, category_id, finance_categories(name, color)")
          .gte(
            "transaction_date",
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `abcuna-relatorio-financeiro-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
                ...
                <BarChart data={monthlyData}>
                  ...
                  <Tooltip
                    contentStyle={{
                      ...
                      border: "1px solid oklch(0.25 0.012 25)",
                      borderRadius: 8,
                    }}
