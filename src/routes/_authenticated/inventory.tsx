                ...
                items.map((i: any) => {
                  ...
                  return (
                    <TableRow key={i.id}>
                      ...
                      <TableCell
                        className={`text-right font-mono text-lg font-bold ${low ? "text-primary" : "text-foreground"}`}
                      >
                        {i.quantity}
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex justify-end gap-1">
                          <Button size="sm" variant="outline" onClick={() => setMovItem(i)}>
                            Movimentar
                          </Button>
                          <Button
                            size="icon"
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!error && created && qty > 0) {
      await supabase
        ...
        .insert({ item_id: created.id, type: "in", quantity: qty, reason: "Estoque inicial" });
    }
    setSaving(false);
    if (error) {
...
function MovementDialog({ item, onDone }: { item: any; onDone: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("inventory_movements")
      .insert({ item_id: item.id, type, quantity: Number(qty), reason });
    setSaving(false);
    if (error) {
      toast.error("Erro", { description: error.message });
      return;
  ...
  return (
    <DialogContent>
      ...
      <p className="text-sm text-muted-foreground">
        Saldo atual: <span className="font-mono font-bold text-foreground">{item.quantity}</span>{" "}
        {item.unit}
      </p>
      <form onSubmit={submit} className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Button
            ...
            onClick={() => setType("out")}
            className={type === "out" ? "bg-primary glow-red" : ""}
          >
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Ex.: uso em atendimento, compra, doação…"
          />
        </div>
        <DialogFooter>
          <Button type="submit" disabled={saving} className="glow-red">
