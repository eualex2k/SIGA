...
function ShiftsPage() {
  ...
  const { data: shifts = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("shifts")
        .select("*, staff(full_name, role_title)")
        .order("shift_date")
        .order("start_time");
                ...
                shifts.map((s: any) => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell>
                      ...
                      <p className="text-xs text-muted-foreground">{s.staff?.role_title}</p>
                    </TableCell>
...
function ShiftStatus({ s }: { s: string }) {
  const map: Record<string, { label: string; cn: string }> = {
    ...
    completed: {
      label: "Concluído",
      cn: "border-emerald-500/40 bg-emerald-500/10 text-emerald-400",
    },
...
function ShiftDialog({ onDone }: { onDone: () => void }) {
  const { data: staff = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("staff").select("id, full_name").eq("active", true).order("full_name"))
        .data ?? [],
  });
