...
function FeesPage() {
  ...
  const filtered = useMemo(
    () =>
      fees.filter((f: any) => {
        ...
        if (
          search &&
          !(f.associates?.full_name?.toLowerCase() ?? "").includes(search.toLowerCase())
        )
          return false;
        return true;
      }),
  ...
  const totalPending = fees
    .filter((f: any) => f.status !== "paid" && f.status !== "cancelled")
  ...
  const markPaid = useMutation({
    mutationFn: async (fee: any) => {
      const { error } = await supabase
        .from("monthly_fees")
        .update({ status: "paid", paid_date: todayISO(), payment_method: "PIX" })
        .eq("id", fee.id);
      if (error) throw error;
      ...
      await supabase.from("finance_transactions").insert({
        type: "income",
        amount: fee.amount,
        description: `Mensalidade - ${fee.associates?.full_name}`,
        category_id: cat?.id,
        transaction_date: todayISO(),
        associate_id: fee.associate_id,
        fee_id: fee.id,
        payment_method: "PIX",
      });
    },
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="p-4">
            ...
            <p className="mt-1 text-xl font-bold text-amber-400">{fmtBRL(totalPending)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-xs uppercase text-muted-foreground">Em atraso</p>
            <p className="mt-1 text-xl font-bold text-primary">{fmtBRL(totalOverdue)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-xs uppercase text-muted-foreground">Recebido (geral)</p>
                ...
                filtered.map((f: any) => (
                  <TableRow key={f.id}>
                    ...
                    <TableCell className="hidden md:table-cell text-sm text-muted-foreground">
                      {f.paid_date ? `${fmtDate(f.paid_date)} (${f.payment_method ?? "—"})` : "—"}
                    </TableCell>
                    <TableCell>
                      <FeeStatus s={f.status} />
                    </TableCell>
...
function GenerateFeesDialog() {
  ...
  const generate = async () => {
    ...
    const { error } = await supabase
      .from("monthly_fees")
      .upsert(rows, { onConflict: "associate_id,reference_month", ignoreDuplicates: true });
    setSaving(false);
    if (error) {
      ...
      <DialogContent>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Será gerada uma cobrança para cada associado ativo, com vencimento dia 10. Duplicidades
            são ignoradas.
          </p>
        </div>
        <DialogFooter>
          <Button onClick={generate} disabled={saving} className="glow-red">
