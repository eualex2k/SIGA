...
function CoursesPage() {
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("course_categories").select("id, name").order("name");
        ...
        actions={
          <Dialog open={open} onOpenChange={(value) => { setOpen(value); if (!value) setEditing(null); }}>
            <DialogTrigger asChild>
              <Button className="glow-red"><Plus className="mr-2 h-4 w-4" /> Novo curso</Button>
            </DialogTrigger>
            <CourseDialog
              categories={categories}
              editing={editing}
              onDone={() => { setOpen(false); setEditing(null); qc.invalidateQueries({ queryKey: ["courses"] }); }}
            />
          </Dialog>
        }
      />
      ...
      <Card>
        <CardContent className="flex flex-col gap-3 p-4 sm:flex-row">
          <div className="relative flex-1">
            ...
            <Input placeholder="Buscar cursos..." value={search} onChange={(event) => setSearch(event.target.value)} className="pl-9" />
          </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={6} className="py-12 text-center text-muted-foreground">Nenhum curso encontrado.</TableCell></TableRow>
              ) : (
                filtered.map((course) => (
                  <TableRow key={course.id} className="group">
                    ...
                    <TableCell className="hidden md:table-cell text-sm text-muted-foreground">{course.category?.name ?? "Sem categoria"}</TableCell>
                    <TableCell className="hidden lg:table-cell text-sm text-muted-foreground">{course.workload_hours}h</TableCell>
                    <TableCell className="hidden xl:table-cell text-sm text-muted-foreground">R$ {Number(course.price).toFixed(2)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="icon" variant="ghost" onClick={() => { setEditing(course); setOpen(true); }}><Pencil className="h-4 w-4" /></Button>
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>Essa ação apagará o curso permanentemente.</AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>Cancelar</AlertDialogCancel>
                              <AlertDialogAction onClick={() => remove.mutate(course.id)}>Excluir</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </div>
...
function CourseDialog({ categories, editing, onDone }: { categories: CategoryOption[]; editing: Course | null; onDone: () => void }) {
    ...
    <DialogContent>
      ...
      <form onSubmit={submit} className="grid gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2"><Label>Carga horária (h)</Label><Input type="number" min={0} value={workload} onChange={(event) => setWorkload(Number(event.target.value))} /></div>
          <div className="space-y-2"><Label>Preço (R$)</Label><Input type="number" min={0} step="0.01" value={price} onChange={(event) => setPrice(Number(event.target.value))} /></div>
        </div>
