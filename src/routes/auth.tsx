  ...
  return (
    <div className="relative grid min-h-screen lg:grid-cols-2">
      ...
      <div className="relative hidden overflow-hidden lg:block">
        <img src={authBg} alt="" className="absolute inset-0 h-full w-full object-cover opacity-60" />
        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-12">
          <AbcunaBrand size="lg" />
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-primary">Central Operacional</p>
            <h1 className="mt-4 text-5xl font-black leading-tight">
              Gestão completa.<br />
              <span className="text-primary">Resposta imediata.</span>
            </h1>
            <p className="mt-4 max-w-md text-sm text-muted-foreground">
              Sistema oficial da Associação de Bombeiros Civis de Uiraúna para o controle integrado de associados, mensalidades, finanças, estoque e escalas operacionais.
            </p>
          </div>
      ...
      <div className="flex items-center justify-center px-6 py-12 sm:px-12">
        <div className="w-full max-w-md">
          ...
          <div className="mt-8 rounded-md border border-primary/30 bg-primary/5 p-4 text-xs text-muted-foreground">
            ...
            <p className="mt-1">E-mail: <span className="font-mono text-foreground">admin@abcuna.org</span></p>
            <p>Senha: <span className="font-mono text-foreground">Abcuna@2026</span></p>
            <p className="mt-2 opacity-80">Cadastre essa conta na aba "Cadastrar" para acessar com permissão de administrador.</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="space-y-2">
        ...
        <Input id="email" type="email" placeholder="operador@abcuna.org" required value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" />
      </div>
      <div className="space-y-2">
        <Label htmlFor="password">Senha</Label>
        <div className="relative">
          <Input id="password" type={show ? "text" : "password"} placeholder="••••••••" required value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" />
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="space-y-2">
        ...
        <Input id="name" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Sgt. Operador Demo" />
      </div>
      <div className="space-y-2">
        <Label htmlFor="email-s">E-mail</Label>
        <Input id="email-s" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@abcuna.org" autoComplete="email" />
      </div>
      <div className="space-y-2">
        <Label htmlFor="pwd-s">Senha (mín. 8 caracteres)</Label>
        <Input id="pwd-s" type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password" />
      </div>
      <Button type="submit" disabled={loading} className="w-full glow-red">
